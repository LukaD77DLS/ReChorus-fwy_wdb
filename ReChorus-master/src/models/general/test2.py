...
class test2(GeneralModel, LightGCNBase):
    reader = 'BaseReader'
    runner = 'BaseRunner'
    ...
    def imix_operation(self, u_embed, i_embed, user):
        ...
        for i in range(len(user)):
            # 随机采样同一用户的另一个物品对
            another_index = random.randint(0, len(user) - 1)
            while another_index == i:
                another_index = random.randint(0, len(user) - 1)

            # 进行 IMix 计算
            lambda_ = np.random.beta(0.5, 0.5)  # 简单示例，可根据实际调整分布参数
            new_u_embed.append(u_embed[i])
            new_i_embed.append(lambda_ * i_embed[i] + (1 - lambda_) * i_embed[another_index])
