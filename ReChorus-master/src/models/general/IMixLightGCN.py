...
class IMixLightGCNBase(LightGCNBase):  
    ...
    def _mix_embeddings(self, u_embed, i_embed):  
        # Mix user and item embeddings  
        batch_size = u_embed.size(0)  
        mixed_user_embeddings = []  
        mixed_item_embeddings = []  

        for b in range(batch_size):  
            # Randomly select a negative sample  
            neg_item_idx = np.random.choice(i_embed.size(0), 1)[0]  
            lambda_val = np.random.uniform(self.lambda_min, self.lambda_max)  

            # Mix embeddings  
            mixed_user = lambda_val * u_embed[b] + (1 - lambda_val) * u_embed[neg_item_idx]  
            mixed_item = lambda_val * i_embed[b] + (1 - lambda_val) * i_embed[neg_item_idx]  

            mixed_user_embeddings.append(mixed_user)  
            mixed_item_embeddings.append(mixed_item)  

        return torch.stack(mixed_user_embeddings), torch.stack(mixed_item_embeddings)  
    ...
    def forward(self, feed_dict):  
        ...
        # Mix user and item embeddings  
        mixed_u_embed, mixed_i_embed = self._mix_embeddings(u_embed, i_embed)  

        # Compute predictions  
        prediction = (mixed_u_embed[:, None, :] * mixed_i_embed).sum(dim=-1)  # [batch_size, -1]  
        return {'prediction': prediction.view(feed_dict['batch_size'], -1)}  

class IMixLightGCN(GeneralModel, IMixLightGCNBase):  
    reader = 'BaseReader'  
    runner = 'BaseRunner'  
    ...
    @staticmethod  
    def parse_model_args(parser):  
        parser = IMixLightGCNBase.parse_model_args(parser)  
        return GeneralModel.parse_model_args(parser)  

    def __init__(self, args, corpus):  
        GeneralModel.__init__(self, args, corpus)  
        IMixLightGCNBase._base_init(self, args, corpus)  

    def forward(self, feed_dict):  
        out_dict = IMixLightGCNBase.forward(self, feed_dict)  
        return {'prediction': out_dict['prediction']}  

class IMixLightGCNImpression(ImpressionModel, IMixLightGCNBase):  
    ...
    def forward(self, feed_dict):  
        return IMixLightGCNBase.forward(self, feed_dict)  
